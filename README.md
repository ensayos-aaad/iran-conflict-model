...
flowchart TD
    subgraph ENTRADAS["ENTRADAS"]
        E1["Operador humano\nMedellín, Colombia"]
        E2["Fuentes web\n~40-60 por sesión"]
        E3["Archivos del repositorio\nREADME / CHANGELOG / simulaciones"]
        E4["Fuente externa K17\nGemini / documentos paralelos"]
        E5["Inyecciones dialécticas\nHipótesis del operador"]
    end

    subgraph PROTOCOLO["PROTOCOLO ANCHOR v2.0"]
        B1["Bloque 1\nModelo a tierra\nautónomo"]
        B2["Bloque 2\nOperador a tierra\nsesgo + agenda"]
        B3["Bloque 3\nVectores\nV-series"]
        B4["Bloque 4\nPausa Sistema 2\nKahneman"]
    ...
    subgraph MOTOR["MOTOR ANALÍTICO"]
        M1["Pearl\nCausalidad / escalera"]
        M2["Kahneman\nSistema 1 vs Sistema 2"]
        M3["Tetlock\nSuperforecasting"]
        M4["Tabla adversarial K3\nR1 contradicción"]
        M5["Clasificación vectores\nE / A / F / U"]
    end

    subgraph KAIZEN["MEJORA CONTINUA — K1-K25"]
        K_A["K1-K8\nCalidad analítica base"]
        K_B["K9-K17\nIntegridad del protocolo"]
        K_C["K18-K25\nDistinciones críticas\nA/F · F/C · sesgos"]
        K_A --> K_B --> K_C
    end

    subgraph ESCENARIOS["ESCENARIOS MODELADOS"]
        SC_F["F — Desescalada narrativa\n0.44 → 0.22"]
        SC_C["C — Escalada / sabotaje\n0.34 → 0.52 DOMINANTE"]
        SC_A["A — Acuerdo formal\n0.10 → 0.06"]
        SC_B["B — Limbo prolongado\n0.10 → 0.10"]
        SC_E["E — Wildcard\n0.04 → 0.06"]
        SC_D["D — Colapso régimen\n0.02 → 0.04"]
    end

    subgraph SALIDAS["SALIDAS POR SESIÓN"]
        S1["Simulación .md\npor sesión"]
        S2["README.md\nactualizado"]
        S3["CHANGELOG.md\nactualizado"]
        S4["Prompt de reinicio\nsiguiente sesión"]
        S5["Convergencia global\n0.87 → 0.51"]
    end

    subgraph REALIMENTACION["REALIMENTACIÓN"]
        RF1["Kaizen nuevos\ndesde errores documentados"]
        RF2["Corrección de sesgo\nK25 + R4"]
        RF3["Ajuste de probabilidades\nK21 umbral de movimiento"]
        RF4["Actualización de protocolo\nANCHOR v1.0 → v2.0"]
    end

    E1 & E2 & E3 & E4 & E5 --> PROTOCOLO
    PROTOCOLO --> MOTOR
    MOTOR --> ESCENARIOS
    M1 & M2 & M3 --> M4 --> M5
    KAIZEN --> MOTOR
    ESCENARIOS --> SALIDAS
    SALIDAS --> RF1 & RF2 & RF3 & RF4
    RF1 & RF2 & RF3 & RF4 --> KAIZEN
    RF4 --> PROTOCOLO
    S1 & S2 & S3 & S4 --> E3

    style SC_C fill:#993C1D,color:#FAECE7
    style SC_F fill:#0C447C,color:#E6F1FB
    style SC_A fill:#085041,color:#E1F5EE
    style REALIMENTACION fill:#F1EFE8
    style KAIZEN fill:#FAEEDA
    style ENTRADAS fill:#E6F1FB
    style SALIDAS fill:#EAF3DE
```
